...
import { ChevronLeft, ChevronRight, Plus, MapPin, Clock, Calendar as CalendarIcon, Trash2, Edit2 } from 'lucide-react';
...
/* ── Main Calendar View ──────────────────────────────── */
export const CalendarView: React.FC<CalendarViewProps> = ({ orgId, groupId }) => {
  ...
  if (!orgId) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-20 text-center animate-in fade-in duration-500">
        <div className="relative mb-8">
          <div className="absolute inset-0 bg-primary/20 blur-3xl rounded-full" />
          <div className="relative w-24 h-24 bg-background/60 backdrop-blur-xl border border-border/40 rounded-[2rem] flex items-center justify-center shadow-2xl shadow-primary/10">
            <CalendarIcon className="w-10 h-10 text-primary" />
          </div>
        </div>
        <div className="space-y-2 max-w-md">
          <h2 className="text-2xl font-black tracking-tight text-foreground">Organization Calendar</h2>
          <p className="text-muted-foreground text-sm font-medium leading-relaxed">
            Select an organization from the sidebar to manage events, schedule meetings, and coordinate with your team.
          </p>
        </div>
      </div>
    );
              ...
              {weekDays.map((day, dayIdx) => {
                ...
                return (
                  <div key={dayIdx} className="flex-1 relative border-l border-border/20"
                    onClick={() => { setSelectedDate(day); setEventToEdit(undefined); setIsEventDialogOpen(true); }}>
                    {/* Hour grid lines */}
                    {HOURS.map(hour => (
